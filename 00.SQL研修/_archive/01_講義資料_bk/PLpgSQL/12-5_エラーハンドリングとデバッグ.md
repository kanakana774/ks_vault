...
PL/pgSQLで独自の例外（Custom Exception）を複数定義し、それを区別してキャッチする方法は、主に2つのアプローチがあります。

PostgreSQLにはJavaやPythonのように「独自の例外クラスを作成する」機能がないため、以下のいずれかの方法で工夫するのが一般的です。

1.  **エラーコード（SQLSTATE）を使い分ける**（推奨・堅牢）
2.  **エラーメッセージ（SQLERRM）で判定する**（手軽・ビジネスロジック向け）

###### 方法1：エラーコード（ERRCODE/SQLSTATE）を使い分ける

これが最もプログラム的に正しい（堅牢な）方法です。
`RAISE` する際に、既存の標準エラーコード（またはユーザー定義用コード）を明示的に指定し、キャッチ側でそのコードに対応する名前（またはコードそのもの）を指定します。

**実装例**
意味的に近い標準エラーコード（例えば `invalid_parameter_value` や `check_violation` など）を借用するか、特定のコードを指定します。

```sql
DO $$
BEGIN
    -- ケース1: 不正な入力として投げる (SQLSTATE: 22023)
    IF true THEN
        RAISE EXCEPTION '入力値が不正です' 
        USING ERRCODE = 'invalid_parameter_value'; -- または '22023'
    END IF;

    -- ケース2: データが見つからないとして投げる (SQLSTATE: P0002)
    IF true THEN
...
EXCEPTION
    ...
    WHEN invalid_parameter_value THEN
        RAISE NOTICE 'パラメータエラーをキャッチしました';
        
    WHEN no_data_found THEN
        RAISE NOTICE 'データなしエラーをキャッチしました';
...
**メリット:**
*   メッセージの内容が変わってもロジックが壊れない。
*   コードの可読性が高い。

**デメリット:**
*   適切なSQLSTATEを探す必要がある（または、5文字のコードを管理する必要がある）。
*   完全に独自のビジネスエラー（例：「在庫不足」と「与信オーバー」）を区別したい場合、適当な標準コードがないことがある。

---

###### 方法2：エラーメッセージで判定する

独自のビジネスロジックエラーが多数あり、標準のSQLSTATE（コード）に当てはめるのが難しい場合によく使われる方法です。
全て `raise_exception` (デフォルトの `P0001`) として投げ、`EXCEPTION` ブロック内でメッセージの中身を見て分岐します。
...
DECLARE
    v_message text;
BEGIN
    -- 何かの条件でエラーAを投げる（特定のプレフィックスを付けると判定しやすい）
    IF true THEN
        RAISE EXCEPTION 'ERR_001: 在庫が足りません';
    END IF;

    -- 別の条件でエラーBを投げる
    IF false THEN
        RAISE EXCEPTION 'ERR_002: アカウントがロックされています';
    END IF;
...
EXCEPTION
    WHEN raise_exception THEN
        ...
        -- メッセージ内容で分岐
        IF v_message LIKE 'ERR_001:%' THEN
            RAISE NOTICE '在庫処理のエラーハンドリングを実行';
        ELSIF v_message LIKE 'ERR_002:%' THEN
            RAISE NOTICE 'アカウントロックの処理を実行';
        ELSE
            -- 想定外のカスタムエラーは再スローする
            RAISE;
        END IF;
...
**デメリット:**
*   エラーメッセージの文言（"ERR_001"など）を変更すると、キャッチ側のロジックも修正が必要になる（密結合）。
*   `IF` 文がネストしてコードが汚れやすい。


**どちらを使うべきか？**

**推奨は「方法1（エラーコード）」と「方法2」の組み合わせ**です。

1.  **明らかな分類ができる場合**（パラメータ不正、データなし、制約違反など）
    → **方法1**（`invalid_parameter_value` などを指定）を使います。

2.  **純粋なビジネスルールの違反**（例：「キャンペーン期間外」「ユーザーランク不足」など）
    → これらはすべて標準エラーとしては「一般的例外」に該当します。
    → 50000番台などのユーザー定義領域のSQLSTATEを使う手もありますが、管理が大変なため、**方法2（特定のプレフィックス付きメッセージ）** で運用するのが現場では一般的です。

**補足**：独自のSQLSTATEを使う場合
PostgreSQLのドキュメントでは、ユーザー定義用エラーコードの厳密な「予約領域」は明記されていませんが、慣習的に標準とかぶらないコード（例: 数字だけの `50001` など）を `USING ERRCODE = '50001'` のように使うことも可能です。ただし、どの数字が何のエラーかをチーム内で厳密に管理する必要があります。
