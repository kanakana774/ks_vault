...
![[00.SQL研修/資料_自分用/01_講義資料/99_画像置き場/window関数1.png]]
![[00.SQL研修/資料_自分用/01_講義資料/99_画像置き場/window関数2.png]]
...
1つの`SELECT`句の中で、ウィンドウ関数ごとに異なる`ORDER BY`句を指定することは全く問題ありません。これはウィンドウ関数の非常に強力な特徴です。
...
SELECT
	d1.sale_date,
	d1.amount,
	COALESCE(d2.amount, 0) AS previous_amount,
	d1.amount - COALESCE(d2.amount, 0) AS 差分
FROM
	daily_sales d1
	LEFT JOIN daily_sales d2 ON d1.sale_date -1 = d2.sale_date
```
