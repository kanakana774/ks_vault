...
さらに、Inventory の在庫数を更新してください（＝注文数を在庫数から差し引く）。在庫はwarehouse_id = 1固定とします。
...
DECLARE
    v_stock INT;
	v_latest_order_id INT;
	v_latest_order_item_id INT;
	v_show_result RECORD;
BEGIN
    -- 在庫確認
    ...
    VALUES (v_latest_order_id, p_customer_id, CURRENT_DATE, 'Pending')
	RETURNING * INTO v_show_result;
	
	RAISE NOTICE '%',v_show_result;

	-- 最新のorder_item_id取得
	SELECT max(order_item_id) + 1
	INTO v_latest_order_item_id
	FROM OrderItems;
    -- 注文明細追加
    INSERT INTO OrderItems (order_item_id, order_id, product_id, quantity)
    VALUES (v_latest_order_item_id, v_latest_order_id, p_product_id, p_quantity)
	RETURNING * INTO v_show_result;
...
```sql
CREATE OR REPLACE PROCEDURE cart_check (
    p_customer_id int,
    p_product_id int,
    p_quantity int,
    p_order_id int -- これは問題的になくてもいいかもしれませんね。。。
) AS $$
DECLARE
    v_total_stock int;
    v_order_id int;
...
BEGIN
    -- 1. 在庫の合計値を確認
    SELECT SUM(stock_quantity) INTO v_total_stock
    FROM inventory
    WHERE product_id = p_product_id;

    -- 2. 在庫不足のチェック（例外を発生させる）
    -- 在庫がNULL（商品が存在しない）または不足している場合
    ...
    -- 3. Ordersへの注文登録
    INSERT INTO orders (order_id, customer_id, order_date, status)
    VALUES (v_order_id, v_customer_id, NOW(), 'Pending')
    RETRUNING order_id INTO v_order_id;
    
    RAISE NOTICE 'INSERT INTO orders : order_id = %', v_order_id;

    -- 最新のorder_item_id取得
    SELECT max(order_item_id) + 1 INTO v_order_item_id FROM orderitems;

    -- 4. OrderItemsに注文明細を追加
    INSERT INTO orderitems (order_item_id, order_id, product_id, quantity)
    VALUES (v_order_item_id, v_order_id, p_product_id, p_quantity);
    
    RAISE NOTICE 'INSERT INTO orderitems : order_id = %', v_order_id;

    -- 5. Inventoryの在庫数を更新（複数の倉庫から引き当てる場合を考慮）
    FOR r_inventory IN 
        SELECT warehouse_id, stock_quantity 
        FROM inventory 
        WHERE product_id = p_product_id 
        ORDER BY warehouse_id -- 更新順序を固定してデッドロックを防止
        FOR UPDATE            -- 行ロックをかけて整合性を確保
    LOOP
        IF v_remaining_qty <= 0 THEN
            EXIT;
        END IF;

        IF r_inventory.stock_quantity >= v_remaining_qty THEN
            -- この倉庫だけで足りる場合
            UPDATE inventory
            SET stock_quantity = stock_quantity - v_remaining_qty
            WHERE warehouse_id = r_inventory.warehouse_id AND product_id = p_product_id;
        ...
        ELSE
            ...
            v_remaining_qty := v_remaining_qty - r_inventory.stock_quantity;
        END IF;
    END LOOP;

    RAISE NOTICE '在庫の更新が完了しました。';
