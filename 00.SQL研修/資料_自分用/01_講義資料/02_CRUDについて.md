...
#### 挿入したデータを受け取る：`RETURNING`句
通常、`INSERT`を実行しても「1件挿入しました」という結果しか返りません。しかし、`RETURNING`句を使うと、**挿入された直後の値（特に自動採番されたIDなど）**をその場で取得できます。

**例：新しく採番された `product_id` を取得する**
```sql
INSERT INTO products (product_name, price)
VALUES ('Webcam', 85.00)
RETURNING product_id;
```
※ アプリケーション開発で、登録した直後のIDを使って別のテーブルにデータを登録したい場合などに非常に重宝します。
