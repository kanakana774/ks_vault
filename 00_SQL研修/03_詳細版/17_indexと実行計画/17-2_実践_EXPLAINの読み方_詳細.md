...
3方式の違いは、**「総当たり（外側の行数 × 内側の行数）をどう避けるか」**の違いだけです。ここは結論の2列だけ押さえてください。

| 方式              | どう結合するか                 | 選ばれるのはこんな時                                |
| :-------------- | :---------------------- | :---------------------------------------- |
| **Nested Loop** | 外側1行ごとに、内側を**インデックスで引く** | 外側が少数 ＋ **内側の結合キーにindexがある**              |
| **Hash Join**   | 内側を**ハッシュテーブル**にして突き合わせる | **大量データ同士**（indexは不要）。ただし**等価結合（`=`）専用**  |
| **Merge Join**  | 双方を**ソートして並走**させる        | 両側が**すでにソート済み**（index順に読める）               |

**3方式に絶対的な優劣はありません。** 「Hash が速い」ではなく、**条件が揃っている方式が選ばれる**だけです。
とくに `Nested Loop` は、**内側の結合キーにインデックスがあるかどうか**で性質が正反対になります（無いと「外側の行数 × 内側の全ページ」の総当たりになる → ケース2）。

> ⚠️ **どれが選ばれるかは、データ量・インデックスの有無・`work_mem` の設定で簡単に入れ替わります。** 「この結合はいつも Hash」と覚えるのではなく、**その場で `EXPLAIN` して確かめてください。**

> 📖 ハッシュテーブルの作り方、`Buckets` の意味、なぜ Hash Join が等価結合専用なのか、`Buffers` が両テーブルのページ数の「和」になる理由 ── アルゴリズムの中身は [[17-3_補足_結合アルゴリズム_詳細|17-3 補足：結合アルゴリズム]] にまとめています。
...
なお、結合方式（Nested Loop / Hash / Merge）の中身が気になった人は [[17-3_補足_結合アルゴリズム_詳細|補足：結合アルゴリズム]] へどうぞ。
