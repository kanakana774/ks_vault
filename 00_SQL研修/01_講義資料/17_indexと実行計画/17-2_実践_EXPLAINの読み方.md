...
> 📖 **なぜハッシュ化すると安くなるのか**、`Buckets` の意味、なぜ Hash Join が等価結合専用なのか、`Buffers` が両テーブルのページ数の「和」になる理由 ── アルゴリズムの中身は [[00_SQL研修/01_講義資料/17_indexと実行計画/17-4_補足_結合アルゴリズム|17-4_補足_結合アルゴリズム]] にまとめています。
