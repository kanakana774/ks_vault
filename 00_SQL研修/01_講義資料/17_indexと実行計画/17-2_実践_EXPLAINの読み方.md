...
> **ここで覚えて帰るのは「インデックス作成はタダではない。稼働中に作るなら `CONCURRENTLY`」の一点で十分です。** 実際に本番で使う場面が来たら、下の折りたたみを開いてください。

> [!example]- 本番で実際に使うときの作法（配属後に読む）
...
> **① 単独で実行すること**
>
> 3つとも**トランザクションブロックの中では実行できません。** クエリツールで他のSQLとまとめて流すと、こうなります。
>
> ```
> ERROR:  CREATE INDEX CONCURRENTLY cannot run inside a transaction block
> ```
>
> **1文だけを選択して実行**してください。これは `VACUUM` と同じ制約です。
>
> **② 他のトランザクションが終わるのを待つ**
>
> `CONCURRENTLY` は、**実行開始時点で走っている全てのトランザクションの終了を待ちます。**「そのテーブルを触っているもの」だけでなく、**無関係なテーブルを触っているものも含めて**です。
>
> 実際に、まったく別のテーブルを更新中のトランザクションを1つ開いたまま実行すると、**それが `COMMIT` するまで一切進みません**（PostgreSQL 17.5 で確認）。
>
> つまり**長時間トランザクションを放置しているシステムでは、`CONCURRENTLY` はいつまでも終わりません。** 実行前に確認してください。
>
> ```sql
...
> **③ 失敗すると「無効なインデックス」が残る**
>
> 途中で失敗しても、**作りかけのインデックスは自動では消えません。** `INVALID` という印が付いた状態で残ります。
>
> ```sql
> -- 無効なインデックスが残っていないか確認
...
> ```
>
> **この状態のインデックスはプランナに使われません。** 検索を速くしないのにディスクを占有するだけなので、見つけたら必ず後始末します。
>
> ```sql
...
> ```
>
> **`CONCURRENTLY` を使ったら、成功したかどうかを必ず上のクエリで確認する**——これを手順に組み込んでおくのが実務の作法です。
...
**既定値はHDD時代の想定です。** SSD環境で既定のままだと、プランナが「ランダムアクセスは高い」と過大評価して、**インデックスがあるのに使わない**判断をすることがあります。**設定を変えるのはサーバー管理者の領域**なので、ここでは「そういう原因もある」と知っておけば十分です。
#### rows（推定行数）
...
**ただし、これは SQL やインデックスではなく「サーバー設定」の領域です。** 設定を変えるのは基本的にサーバー管理者やDBA担当の仕事で、**自分で勝手に変えるものではありません。**

皆さんにとって大事なのは、**「SQLもインデックスも正しいのに、まだ Seq Scan が選ばれる」というときに、原因がその外側にある可能性を知っておくこと**です。そこまで来たら、**先輩やDBA担当に「`random_page_cost` が実機と合っていないかもしれない」と相談できれば十分**です。

> 💡 **調べるときの道具だけ紹介しておきます。**
> ```sql
> SET enable_seqscan = off;    -- Seq Scan を一時的に禁止して、別の計画のコストを見る
> EXPLAIN (ANALYZE, BUFFERS) SELECT ... ;
> RESET enable_seqscan;        -- 必ず戻す
> ```
> **セッション限定なので安全**です。「プランナはなぜこっちを選んだのか」を調べるときに使えます。
> ただしこれは**調査用**であって、「禁止したら速くなったから」と本番で設定するものではありません。
...
**`Sort` ノードが出ていたら、インデックスで消せないか**を考える癖をつけてください（§3-2 のノード一覧でも触れた通りです）。ソートは大量データでは `work_mem` を食い、溢れるとディスクを使います（→ 演習 問題8）。
...
9.  **それでもおかしいなら、コスト設定を疑う。** ここから先はサーバー設定の話なので、**先輩やDBA担当に相談**してください（ケース5）。
...
なお、結合方式（Nested Loop / Hash / Merge）の中身が気になった人は [[17-4_補足_結合アルゴリズム|17-4 補足：結合アルゴリズム]] へどうぞ。
