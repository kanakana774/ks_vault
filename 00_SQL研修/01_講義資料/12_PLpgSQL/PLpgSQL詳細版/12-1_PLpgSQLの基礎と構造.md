...
*   **大文字小文字**: SQLと同様、キーワードはケースインセンティブ（大文字小文字を区別しない）ですが、可読性のためにキーワードは大文字で書くのが一般的です。
...
## 1.5 変数と識別子の衝突：#variable_conflict

現場でよくあるトラブルの一つに、**「PL/pgSQLの変数名」と「テーブルの列名」が同じ**場合に、どちらが優先されるかという問題があります。

デフォルトでは、衝突した場合にエラー（Ambiguous column）になるか、予期せぬ挙動をします。これを制御するのが `#variable_conflict` 指示子です。

### 実演：列名と変数名の衝突制御

```sql
DO $$
#variable_conflict use_variable -- 変数名を優先する設定（デフォルトは error）
DECLARE
    emp_id integer := 1;
    v_name text;
BEGIN
    -- ここで emp_id は変数の 1 と解釈される
    -- WHERE emp_id = emp_id は WHERE 従業員のID = 1 と解釈される
    SELECT name INTO v_name 
    FROM employees 
    WHERE emp_id = employees.emp_id;

    RAISE NOTICE 'Found name: %', v_name;
END;
$$ LANGUAGE plpgsql;
```

*   `use_variable`: 衝突時にPL/pgSQL変数を優先。
*   `use_column`: 衝突時にテーブルの列名を優先。
*   `error`: 衝突時にエラーにする（推奨される安全なデフォルト）。

**エンジニア向けのヒント**: そもそも衝突を避けるため、変数には `v_` (variable) や `p_` (parameter) といったプレフィックスを付けるのが業界のベストプラクティスです。
...
*   ブロックはネスト可能。ラベルを使うことで、ネストしたスコープ間の変数操作を明確にできる。
*   セミコロンの打ち忘れや `$$` の使い方など、構文の基本を疎かにしないことが、実行時エラーを減らす近道である。
