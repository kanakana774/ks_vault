...
単価がどれも同じで問題にならないので、再生時間に問題変更

問題：
`track` テーブルから、各ジャンル (`genre_id`) ごとに、`milliseconds` （再生時間）の値がジャンル内で最も高いものから 3 番目までのトラックを取得してください。
...
FROM (
    SELECT
        ...
        genre_id,
        milliseconds,
        DENSE_RANK() OVER (PARTITION BY genre_id ORDER BY milliseconds DESC) AS 再生時間ランキング
    FROM track
) AS ranked_tracks
WHERE 再生時間ランキング <= 3
ORDER BY genre_id, 再生時間ランキング, track_id;
```
---

**問題 3**
`employee` テーブルから、各従業員について、同じ`title`（肩書）を持つ従業員の中での勤続期間（`hire_date`と`CURRENT_DATE`の差）と同じ`title`内でのその順位と、同じ`title`を持つ従業員の中で最も勤続年数が長い従業員を出力してください。
...
下記のようにwindowフレーム指定しないと3つ以上履歴があっても上から２番目まではnullででてしまうので、下記が正確。
逆に顧客に３件以上の注文がない場合は、nullが帰り、要件を満たす。
...
SELECT
	...
	customer_id,
	invoice_date,
	NTH_VALUE(invoice_date, 3) OVER (
		PARTITION BY
			customer_id
		ORDER BY
			invoice_date DESC ROWS BETWEEN unbounded preceding
			AND unbounded following
	) AS third_latest_invoice_date
FROM
	invoice
ORDER BY
	customer_id,
...
`album_id`, `media_type_id`, `genre_id`はそれぞれ仮の値（適当な値）を使用してください。
