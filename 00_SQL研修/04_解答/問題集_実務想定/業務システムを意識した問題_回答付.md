...
WITH
	over_budget_task AS (
		...
		FROM
			tasks AS t
			JOIN timesheets AS ts ON t.task_id = ts.task_id
		GROUP BY
			t.task_id
		HAVING
			t.estimated_hours < SUM(ts.actual_hours)
	)
SELECT
	p.project_name,
	COUNT(DISTINCT pm.employee_id) AS member_count, -- まだアサインされたけど作業実績がない人をカウントするためにpm.employee_idでカウント
	COUNT(DISTINCT t.task_id) AS task_count,
	COALESCE(SUM(ts.actual_hours), 0) AS total_actual_hours,
	COUNT(DISTINCT obt.task_id) AS over_budget_task_count
FROM
	projects AS p
	JOIN tasks AS t ON p.project_id = t.project_id
	LEFT JOIN timesheets AS ts ON t.task_id = ts.task_id -- 着手してないタスクがある可能性があるためleft join
	LEFT JOIN over_budget_task AS obt ON obt.task_id = t.task_id -- 予定超過してないタスクも残すためleft join
	LEFT JOIN project_members AS pm ON p.project_id = pm.project_id AND ts.employee_id = pm.employee_id -- まだアサインされたけど作業実績がない人をカウントするため
GROUP BY
	p.project_id
ORDER BY
	p.project_id
...
WITH
	実績工数超過 AS (
		...
		FROM
			projects プロジェクト
			INNER JOIN tasks タスク ON プロジェクト.project_id = タスク.project_id
			INNER JOIN timesheets 実績 ON 実績.task_id = タスク.task_id
		GROUP BY
			プロジェクト.project_id,
			タスク.task_id
		HAVING
			SUM(実績.actual_hours) >= タスク.estimated_hours
		ORDER BY
			プロジェクト.project_id,
			タスク.task_id
	)
...
WHERE
	EXISTS (
		...
		FROM
			実績工数超過
		WHERE
			実績工数超過.project_id = プロジェクト.project_id
	)
```
...
WITH
	顧客ごとの購入金額 AS (
		SELECT
			顧客.customer_id 顧客id,
			顧客.customer_name 顧客名,
			顧客.is_deleted 削除フラグ,
			COALESCE(
				SUM(
					(注文明細.unit_price_at_order * 注文明細.quantity) - COALESCE(注文明細.discount_amount, 0)
				),
				0
			) AS "合計購入金額"
		FROM
			customers 顧客
			LEFT OUTER JOIN sales_orders 注文 ON 顧客.customer_id = 注文.customer_id
			LEFT OUTER JOIN sales_details 注文明細 ON 注文明細.order_id = 注文.order_id
		WHERE
			(
				注文.order_status <> 9
				OR 注文.order_status IS NULL
			)
			AND 顧客.is_deleted = 'FALSE'
		GROUP BY
			顧客.customer_id,
			顧客.customer_name,
			顧客.is_deleted
...
SET
	rank_id = (
		SELECT
			CASE
				WHEN 合計購入金額 < 500 THEN NULL
				WHEN 合計購入金額 < 1000 THEN 10
				ELSE 20
			END AS ランク
		FROM
			顧客ごとの購入金額
		WHERE
			顧客ごとの購入金額.顧客id = 顧客.customer_id
	)
WHERE
	is_deleted <> TRUE
```
