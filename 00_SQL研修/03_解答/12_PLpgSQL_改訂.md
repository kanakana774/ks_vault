# 解答：PL/pgSQL（DO ブロック編）

> **この解答について**
> - 問題は [[00_SQL研修/02_問題/12_PLpgSQL_改訂|02_問題/12_PLpgSQL_改訂]] です。
> - PostgreSQL 17 で全問実行して確認しています。掲載している出力は**すべて実測**です。
> - 使用するテーブルは [[00_SQL研修/02_問題/08_結合から使用するテーブル|08_結合から使用するテーブル]] の初期状態です。
> - **配布厳禁。**
...
`FOR` ループで 1 から 5 までを回し、各ループでその評価（`reviews.rating`）のレビュー件数を数えて、`評価[N]: [件数]件` の形式で出力する `DO` ブロックを作ってください。
...
`products` と `categories` を結合し、すべての商品について「商品名」「カテゴリ名」「価格」を取得します。`FOR ... IN SELECT ... LOOP` で 1 行ずつ `商品: [商品名], カテゴリ: [カテゴリ名], 価格: [価格]` の形式で出力する `DO` ブロックを作ってください。
...
すべての顧客（`customers`）についてループし、各顧客の注文合計金額を計算して `顧客名: [顧客名], 注文合計金額: [合計金額]` の形式で出力する `DO` ブロックを作ってください。
