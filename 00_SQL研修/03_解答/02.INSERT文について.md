...
PostgreSQLでは `RETURNING` 句を使うと便利ですが、ここでは手順を追って解説します。
...
VALUES (2, '2024-02-01')
RETURNING order_id;
-- 実行結果例: order_id = 12 が返されたと仮定

-- 手順2: 取得した order_id (例: 12) を使用して order_details_trn に明細を登録
...
> **解説**: 実務のアプリケーション開発では、プログラムが 1 の結果（ID）を受け取り、それを 2 の SQL に埋め込んで実行します。
> `RETURNING` 句は PostgreSQL などの一部のDBで使える便利な機能で、INSERT した直後にその行の特定のカラム（ここでは自動採番されたID）を返却させることができます。
