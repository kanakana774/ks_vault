...
**問題 8**
`customer` テーブルから、`country` が 'Brazil' の顧客の `first_name` と `last_name` を取得し、それと `employee` テーブルから `country` が 'Canada' の従業員の `first_name` と `last_name` を結合して表示してください。重複は排除してください。
