...
これから実装するWeb APIが、なぜ「Controller」「Service」「Mapper」といった複数のクラスに分割されているのか、その設計思想と実装上のルールを解説するものです。この「型」を理解することが、保守性が高く、チームで開発しやすいアプリケーションを作るための第一歩となります。
...
この場合、`Form`と`Entity`は以下のように責務に応じて設計されます。
